(function () {
  ...
  function initUiControls() {
    ...
    // Quick theme control in the shared suite sidebar
    const quickThemeToggle = byId("quickThemeToggle");
    const quickThemeText = byId("quickThemeText");
    const quickThemeIcon = byId("quickThemeIcon");

    function syncQuickThemeButton() {
      const current = document.documentElement.dataset.theme || resolvedTheme(localStorage.getItem("regTool.theme") || "dark");
      if (quickThemeText) quickThemeText.textContent = current === "dark" ? "Cambiar a claro" : "Cambiar a oscuro";
      if (quickThemeIcon) quickThemeIcon.textContent = current === "dark" ? "☀" : "☾";
    }
    ...
    if (quickThemeToggle) {
      quickThemeToggle.addEventListener("click", function () {
        ...
        applyTheme(next);
        syncQuickThemeButton();
      });
    }
    if (theme) theme.addEventListener("change", syncQuickThemeButton);
    syncQuickThemeButton();
